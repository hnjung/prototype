...
const commUiFnFixMainButton = {
	init : () => {
		...
		mainButton.forEach(function(_this) {
			...
			const thisFooter = thisPage.querySelector('#footer');
			if (thisFooter) {
				commUiFnFixMainButton.add(thisPage);
				if (_this.classList.contains('option-react')) {
					thisPage.classList.add('option-react');
				}
			} else {
				/* 탭 안에 버튼이 있는 경우 */
...
const commUiFnContentFix = {
	init : () => {
		...
		mainButton.forEach(function(_this) {
			...
			let thisFooter;
			if (thisPage) {
				thisFooter = thisPage.querySelectorAll('#footer');
			}
			const section = thisParent.querySelectorAll('.section');
			const thisHeight = _this.scrollHeight;
			if (thisParent) {
				if (thisFooter) {
					commUiFnContentFix.set(thisFooter, thisHeight);
				} else {
...
const commUiFnGNB = {
	...
	mobile : () => {
		...
		if (header) {
			...
			if (gnbMenuClose) {
				gnbMenuClose.addEventListener('click', function() {
					const headerClass = header.classList;
	...
	tablet : () => {
		...
		const gnbSaveBtn = document.querySelector('.gnb-btn-wrap');
		if (gnb) {
			const gnbClass = gnb.classList;
			if (gnbItem) {
				gnbItem.forEach(function(_this) {
					/* 1뎁스 클릭 시 */
					_this.querySelector('.txt').addEventListener('click', function() {
						if (!header.classList.contains(commUiVar.openClass)) {
							if (gnbClass.contains(commUiVar.openClass)) {
								gnbWrap.style.maxHeight = `6.4rem`;
								gnbClass.remove(commUiVar.openClass);
							} else {
								gnbWrap.style.maxHeight = `${gnbList.scrollHeight}px`;
								gnbSaveBtn.style.top = `calc(${gnbList.scrollHeight}px - 6.4rem)`;
								gnbClass.add(commUiVar.openClass);
							}
						}
					});
					/* 1뎁스 키 입력 시 클릭 이벤트 연결 */
					_this.querySelector('.txt').addEventListener('keyup', function(e) {
						if (e.keyCode === 32 || e.keyCode === 13) {
							this.closest('.gnb-list').querySelectorAll('.fn-fold-link').forEach(function(_this) {
								_this.click();
							});
						}
					});
			...
			/* 딤드 클릭 시 */
			gnb.addEventListener('click', function(_event) {
				if (_event.target.id === 'gnb' && _event.target.classList.contains(commUiVar.openClass)) {
					gnbWrap.style.maxHeight = `6.4rem`;
					gnbClass.remove(commUiVar.openClass);
				}
			});

			const setManager = gnb.querySelector('.util-mypage .btn-set-member');
			if (gnb.classList.contains('option-manager') && setManager) {
				setManager.addEventListener('click', function() {
					if (gnb.classList.contains('open-manager')) {
						gnb.classList.remove('open-manager');
					} else {
						gnb.classList.add('open-manager');
					}
				});
			}
		}
		...
		window.addEventListener('resize', function() {
			...
			if (header) {
				...
				if (gnb) {
					...
					const gnbWrap = document.querySelector('.gnb-wrap');
					const gnbSaveBtn = document.querySelector('.gnb-btn-wrap');
					gnbWrap.setAttribute('style', '');
					gnbSaveBtn.setAttribute('style', '');
	
					/* 해상도별 메뉴 초기화 */
					if (header.offsetWidth < 1024) {
						gnb.querySelectorAll('.gnb-list .fn-fold-wrap > .fn-fold-btn').forEach(function(_this) {
							commUiFnFold.set(_this, true);
						});
					} else {
						gnb.querySelectorAll('.gnb-list .fn-fold-wrap > .fn-fold-btn').forEach(function(_this) {
							commUiFnFold.set(_this, false);
						});
					}
				}
			}
...
/* 레이아웃-북마크 - 2024-03-29 : 위치 이동 */
const commUiFnBookmark = {
	/* 초기실행 */
	init : () =>  {
		commUiFnBookmark.tablet();
	},
	/* 태블릿 */
	tablet : () => {
		const header = document.querySelector('#header');
		...
		if (bookmarkBtn) {
			const bookmarkClass = bookmark.classList;
			/* 버튼 클릭 시 */
			bookmarkBtn.addEventListener('click', function() {
				if (bookmarkClass.contains(commUiVar.openClass)) {
					bookmarkClass.remove(commUiVar.openClass);
					header.classList.remove('is-bookmark');
				} else {
					bookmarkClass.add(commUiVar.openClass);
					header.classList.add('is-bookmark');
				}
			});

			/* 딤드 클릭 시 */
			bookmark.addEventListener('click', function(_event) {
				if (_event.target.classList.contains('bookmark') && _event.target.classList.contains(commUiVar.openClass)) {
					bookmarkClass.remove(commUiVar.openClass);
					header.classList.remove('is-bookmark');
				}
			});
...
commUiFnBookmark.init();

/* 레이아웃-푸터 - 2024-03-29 : 위치 이동 */
const commUiFnFooter = {
	/* 초기실행 */
	init : () =>  {
		commUiFnFooter.set();
	},
	...
	set : () => {
		...
		if (footer) {
			const footerClass = footer.classList;
			const footerDetail = footer.querySelector('.footer-detail');
			const btnFooter = footer.querySelector('.btn-footer-info');
			if (btnFooter) {
				btnFooter.addEventListener('click', function() {
					if (footerClass.contains(commUiVar.openClass)) {
						footerDetail.style.maxHeight = `0px`;
						footerClass.remove(commUiVar.openClass);
					} else {
						footerDetail.style.maxHeight = `${footerDetail.scrollHeight}px`;
						footerClass.add(commUiVar.openClass);
					}
				});
			}
		}
