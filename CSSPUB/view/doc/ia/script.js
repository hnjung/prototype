...
$.each(iaMainLoginData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaProductData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaCustomData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaContractData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaSalesData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaComponentSubscriptionData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaComponentEtcData.list , function(){
	iaAllData.list.push(this);
});
$.each(iaEtcData.list , function(){
	iaAllData.list.push(this);
});
...
/* 카테고리별 데이터 가져오기 */
/** 메인/로그인 */
const mainLoginData = JSON.parse(JSON.stringify(iaMainLoginData));
iaData.set('tabMainLogin', mainLoginData);

/** 상품보기 */
const productData = JSON.parse(JSON.stringify(iaProductData));
iaData.set('tabProduct', productData);

/** 고객관리 */
const customData = JSON.parse(JSON.stringify(iaCustomData));
iaData.set('tabCustom', customData);

/** 계약관리 */
const contractData = JSON.parse(JSON.stringify(iaContractData));
iaData.set('tabContract', contractData);

/** 영업지원 */
const salesData = JSON.parse(JSON.stringify(iaSalesData));
iaData.set('tabSales', salesData);

/** 청약확인(컴포넌트) */
const componentSubscriptionData = JSON.parse(JSON.stringify(iaComponentSubscriptionData));
iaData.set('tabComponentSubscription', componentSubscriptionData);

/** 기타(컴포넌트) */
const componentEtcData = JSON.parse(JSON.stringify(iaComponentEtcData));
iaData.set('tabComponentEtc', componentEtcData);

/** 기타 */
const etcData = JSON.parse(JSON.stringify(iaEtcData));
iaData.set('tabEtc', etcData);
